...
def build_profile(wallet, score_payload: dict, *, brand: dict | None = None) -> dict:
    ...
    credit = {
        ...
        "rationale": [
            {"label": "How often you get paid", "rating": _rating(scores["volume"])},
            {"label": "Steady money in and out", "rating": _rating(scores["cash-flow"])},
            {"label": "Paying back on time", "rating": _rating(scores["repayment"])},
            {"label": "Different people you pay", "rating": _rating(scores["counterparties"])},
            {"label": "Warning signs", "rating": _rating(scores["risk-activity"], inverted=True)},
        ],
        "methodology": [
            {"id": "wallet-age", "label": "How long you have used this wallet", "weightPercent": 15, "scoreOutOf100": scores["wallet-age"]},
            {"id": "volume", "label": "How often money moves", "weightPercent": 25, "scoreOutOf100": scores["volume"]},
            {"id": "cash-flow", "label": "Steady money in and out", "weightPercent": 20, "scoreOutOf100": scores["cash-flow"]},
            {"id": "repayment", "label": "Paying back on time", "weightPercent": 20, "scoreOutOf100": scores["repayment"]},
            {"id": "counterparties", "label": "Different people you pay", "weightPercent": 10, "scoreOutOf100": scores["counterparties"]},
            {"id": "risk-activity", "label": "Returned or disputed payments", "weightPercent": 10, "scoreOutOf100": scores["risk-activity"]},
        ],
        "source": source,
    }
    risk_analysis = {
        ...
        "factors": [
            {"id": "cash-flow", "label": "Steady money in and out", "rating": _rating(scores["cash-flow"])},
            {"id": "repayment", "label": "Paying back on time", "rating": _rating(scores["repayment"])},
            {"id": "diversity", "label": "Different people you pay", "rating": _rating(scores["counterparties"])},
            {"id": "concentration", "label": "Too much money from one place", "rating": _rating(scores["counterparties"])},
            {"id": "suspicious", "label": "Unusual payments", "rating": _rating(scores["risk-activity"], inverted=True)},
            {"id": "circular", "label": "Money sent in a circle", "rating": _rating(scores["risk-activity"], inverted=True)},
        ],
        "source": source,
    ...
    report = {
        ...
        "basis": "This summary is based on payments we could see in your wallet.",
